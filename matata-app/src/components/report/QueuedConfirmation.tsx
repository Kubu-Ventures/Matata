...
export default function QueuedConfirmation({
  ...
  refId: string;
  /** When provided (inline usage right after an offline submit), reset
   *  local state instead of navigating -- a route change needs a network
   *  fetch for the destination's data, which is exactly what isn't
   *  available yet and is how reporters ended up stuck on the PWA's
   *  static offline fallback page right after submitting offline. */
      ...
      <div className="max-w-sm w-full text-center space-y-6">
        ...
        {refId && (
          <div className="bg-[#EDEFF0] rounded-lg px-4 py-3">
            ...
            <p className="text-sm font-mono font-medium text-[#232E3D] break-all">{refId}</p>
          </div>
        )}
