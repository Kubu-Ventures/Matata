...
export function LanguageSelector() {
  ...
  return <label className="inline-flex items-center gap-2 border border-line bg-surface px-3 text-sm font-semibold text-navy"><Globe2 size={17} aria-hidden="true" /><span className="sr-only">{translate('language')}</span><select aria-label={translate('language')} value={locale} onChange={(e) => setLocale(e.target.value as Locale)} className="min-h-10 cursor-pointer bg-transparent py-2 outline-none focus-visible:ring-2 focus-visible:ring-udnp-blue">{Object.entries(LOCALES).map(([key, value]) => <option key={key} value={key}>{value.flag} {value.label}</option>)}</select></label>
}

export function Header() {
  const { translate } = useLanguage()
  return <header className="border-b-4 border-udnp-red bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8"><Link href="/" className="flex min-w-0 items-center gap-3" aria-label={translate('brand')}><span className="grid h-10 w-10 shrink-0 place-items-center bg-udnp-red text-lg font-black text-white">M</span><span className="truncate text-xl font-black tracking-[0.18em] text-navy">{translate('brand')}</span></Link><div className="flex items-center gap-3"><LanguageSelector /><Link href="/login" className="hidden min-h-10 items-center border border-udnp-blue px-4 py-2 text-sm font-bold text-udnp-blue transition hover:bg-udnp-blue hover:text-white sm:inline-flex">{translate('signIn')}</Link></div></div></header>
}

export function Footer() { const { translate } = useLanguage(); return <footer className="border-t border-line bg-navy text-white"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 text-sm sm:flex-row sm:items-center sm:justify-between lg:px-8"><div><p className="font-bold tracking-[0.12em]">{translate('brand')}</p><p className="mt-2 max-w-md text-white/70">{translate('privacy')}</p></div><Link href="/analyst/login" className="font-semibold text-white underline decoration-udnp-red decoration-2 underline-offset-4">{translate('analyst')}</Link></div></footer> }
