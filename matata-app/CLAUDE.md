...
Matata is a Next.js 16 (App Router) PWA for crowdsourced crisis/disaster damage reporting. It talks to an external backend API (`NEXT_PUBLIC_API_URL`, default `http://localhost:8000/api/v1`). There is no backend code in this repo; the backend is `Kubu-Ventures/Matata-backend`.

### Two user-facing surfaces

- **Public reporter flow** (`src/app/page.tsx`, `src/app/report/*`, `src/app/login/*`): anonymous or email-verified citizens submit a multi-step damage report (location, crisis type, details, photo, review) via `src/app/report/page.tsx`.
...
Token, refresh token, and role are stored directly in `localStorage` (`matata_token`, `matata_refresh`, `matata_role`). No cookies, no server session. `Role` is one of `anonymous_reporter | reporter | analyst | responder | admin` (`src/lib/types.ts`). Reporters get an anonymous session token from `authApi.anonymous()` on first visit to `/report`. Signed-in users (email reporters and analysts) go through Privy hosted email OTP: `LoginForm` runs `useLoginWithEmail`, then `exchangePrivySession()` (`src/lib/privyLogin.ts`) posts the Privy tokens to `authApi.verifyPrivy` (`POST /auth/privy/verify`) and stores the returned session. The backend resolves the role from the verified email. `PrivyClientProvider` (`src/components/PrivyClientProvider.tsx`) mounts `PrivyProvider` only after hydration and is skipped if `NEXT_PUBLIC_PRIVY_APP_ID` is unset. Full flow: `docs/auth.md`.
